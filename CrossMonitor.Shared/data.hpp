...
public:
	 ...
	 */
	data(float cpu_percent, float memory_percent, unsigned process_count, uint64_t io_usage) {
		set_cpu_percent(cpu_percent);
		set_memory_percent(memory_percent);
		set_process_count(process_count);
		set_io_usage(io_usage);
	}
	...
	*/
	void set_io_usage(uint64_t io_usage) noexcept {
		io_usage_ = io_usage;
	}
	uint64_t get_io_usage() const noexcept {
		return io_usage_;
	}
