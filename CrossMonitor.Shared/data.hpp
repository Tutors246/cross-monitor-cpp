...
public:
	 ...
	 */
	data(int cpu_percent, int memory_percent, unsigned process_count, int io_percent) {
		set_cpu_percent(cpu_percent);
		set_memory_percent(memory_percent);
		set_process_count(process_count);
		set_io_percent(io_percent);
	}
	 ...
	 * @param cpu_percent CPU use percentage (0 to 100).
	 */
	void set_cpu_percent(int cpu_percent) {
	...
	int get_cpu_percent() const noexcept {
		return cpu_percent_;
	}
	...
	*/
	void set_memory_percent(int memory_percent) {
	...
	int get_memory_percent() const noexcept {
	...
	*/
	...
	/**
	* Setter.
	* @param io_usage Total read and write operation.
	*/
	void set_io_percent(int io_percent) noexcept {
		io_percent_ = io_percent;
	}
	int get_io_percent() const noexcept {
		return io_percent_;
	}
